...
class MemberController extends ComController
{
    public function index()
    {
        ...
        $list = $user->field("{$prefix}member.*,{$prefix}auth_group.id as gid,{$prefix}auth_group.title,{$prefix}org.org_name")
            ->order($order)
            ->join("{$prefix}auth_group_access ON {$prefix}member.uid = {$prefix}auth_group_access.uid")
            ->join("{$prefix}auth_group ON {$prefix}auth_group.id = {$prefix}auth_group_access.group_id")
            ->join("{$prefix}org ON {$prefix}member.org_id = {$prefix}org.id")
            ->where($where)
    ...
    public function update($ajax = '')
    {
        ...
        $data['org_id'] = isset($_POST['org_id']) ? trim($_POST['org_id']) : '';
    ...
    public function add()
    {
        ...
        $org = M("org")->field('id,org_name')->where("is_delete = 0")->select();
        $usergroup = M('auth_group')->field('id,title')->select();
        $this->assign('usergroup', $usergroup);
        $this->assign('org',$org)->display('form');
    }
}
